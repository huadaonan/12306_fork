...
function injectDom() {
	...
	html.push('<h4 style="font-size:18px; font-weight:bold; margin: 0px; line-height: 26px; border-bottom: 1px dotted #ccc;">12306 订票助手  （anarch） <small>ver ' + window.helperVersion + '</small></h4>');
	...
	html.push('<table>');
	html.push('<tr>');
	html.push('<td colspan="4">1.放票非正点也，在将近放票的时候，务必保持刷新状态，而且……当整点没有放票时，不要放弃继续刷新；<br />2.动车都是11点放票，切记切记；<br />3.第一波放票没有订到时，请耐心等待，因为现在放票有N多节点，随时会有票出来，晚很久才放票也正常；<br />\
');
	html.push('</td></tr>');
	html.push('</table>');
	html.push('</div><div class="tabLog tabContent"><div>下面是当前页面的记录。如果您的助手遇到功能上的问题，请全部复制后发成邮件给作者：ifish@fishlee.net 。<span style="color:red;font-weight:bold;">请在发送前务必剔除记录中包含的个人隐私如密码等信息。</span></div><textarea id="runningLog" style="width:100%;height:200px;"></textarea></div>');
	//获取登录到IE的代码 Add By XPHelper
	html.push('<div class="tabLoginIE tabContent"><div><strong>先在IE中打开 https://dynamic.12306.cn/otsweb，</strong>再将以下代码复制到IE浏览器的地址栏。确认地址栏最前面有“<strong>javascript:</strong>”字样，没有请手动加上，然后敲回车，等待页面刷新后，即可自动登录。</div><textarea id="LoginIECode" style="width:100%;height:200px;"></textarea></div>');
...
var utility = {
	...
	getAudioUrl: function () {
		return "http://static.liebao.cn/resources/audio/song.ogg";
	},
	getFailAudioUrl: function () {
		return "http://static.liebao.cn/resources/audio/music3.ogg";
	},
	...
	getUpdateUrl: function () {
		return "http://www.fishlee.net/Service/Download.ashx/44/69/12306_ticket_helper_for_liebaobrowser.crx";
	}
}
	...
	(function () {
		...
		function doCheck() {
			...
			utility.get("https://dynamic.12306.cn/otsweb/order/myOrderAction.do?method=getOrderWaitTime&tourFlag=" + flag, null, "json", function (data) {
				...
				if (data.waitTime > 0) {
					obj.css({ "color": "red" }).html("排队中<br />排队数【" + (data.waitCount || "未知") + "】<br />预计时间【" + utility.getSecondInfo(data.waitTime) + "】");
				} else {
...
function autoReloadIfError() {
	if ($.trim($("h1:first").text()) == "错误") {
		$("h1:first").css({ color: 'red', 'font-size': "18px" }).html("&gt;_&lt; 2秒后重新登录");
		setTimeout(function () {
			self.location.reload();
		}, 2000);
	}
		...
		$.ajax({
			...
			success: function (json) {
				...
				if (json.waitTime == -1 || json.waitTime == 0) {
					...
					var msg = "很抱歉, 订单被撤销了, 请赶紧重新下单!";
					utility.notify(msg);
					setCurOperationInfo(false, msg);
					stop(msg);
					reloadCode();
				} else if (json.waitTime == -2) {
					var msg = "很抱歉, 订单失败 : " + json.msg + ', 请赶紧重新下单!';
					reloadToken();
					utility.notify(msg);
					setCurOperationInfo(false, msg);
					stop(msg);
					...
					var msg = "订单需要 " + utility.getSecondInfo(json.waitTime) + " 处理完成，请等待。（排队人数=" + (json.waitCount || "未知") + "）";
					if (json.waitTime > 1800) {
						msg += "<span style='color:red; font-weight: bold;'>警告：排队时间大于30分钟，请尝试电话订票！</span>";
					}
					setTipMessage(msg);

					if (json.waitTime > 1800 && !waitTimeTooLong_alert) {
						waitTimeTooLong_alert = true;
						utility.notifyOnTop("警告！排队时间大于30分钟，成功率较低，请尽快电话订票！");
					}
...
function initTicketQuery() {
	...
	if (!window.localStorage["pas"]) {
		alert("请进入【我的12306->常用联系人】并稍等片刻以更新缓存~");
	...
	form.find("tr:last").after("<tr class='append_row'><td colspan='9' id='queryFunctionRow'><label title='勾选后，每次你查询后，助手会帮你把始发站、到达站、日期等进行记录，下次进入查询页面后，将会帮您自动填写好'><input type='checkbox' id='keepinfo' checked='checked' />记住信息</label> <label title='勾选后，假定查询的结果中没有符合你要求的车次，助手将会自动进行重新查询'><input checked='checked' type='checkbox' id='autoRequery' />自动重新查询</label>，每隔 <input style='width:50px;' type='number' min='1' value='5' size='4' id='refereshInterval' style='text-align:center;' />秒(最低5) " +
		"<label title='勾选的话，当有票可定时，助手会放歌提示你'><input type='checkbox' checked='checked' id='chkAudioOn'>声音提示</label> <input type='button' id='chkSeatOnly' value='仅座票' class='lineButton' title='勾选后可快速勾选所有的座票，包括硬座软座一等座等等' /><input type='button' id='chkSleepOnly' value='仅卧铺' title='勾选后可快速勾选所有的卧铺，包括硬卧软卧' class='lineButton' /><input type='button' id='chkAllSeat' value='全部席别' class='lineButton' title='快速勾选所有的席别' />" +
		...
		"<tr class='append_row'><td colspan='9'><label title='勾选后将在有票时循环播放歌曲'><input type='checkbox' checked='checked' id='chkAudioLoop'>声音循环</label>" +
		"<span style='font-weight:bold;margin-left:10px;color:blue;' title='点击预定按钮时，有时候等待一会儿系统会提示服务器忙；勾选后，如果出现这种情况，助手将会进行自动重新预定'><label><input type='checkbox' id='chkAutoResumitOrder' checked='checked' />预定失败时自动重试</label></span>" +
		"<span style='font-weight:bold;margin-left:10px;color:blue;' title='有时候系统忙，查询会提示查询失败；勾选后，如果出现这种情况，助手将会进行自动刷新查询'><label><input type='checkbox' id='chkAutoRequery' checked='checked' />查询失败时自动重试</label></span>" +
		"<span style='font-weight:bold;margin-left:10px;color:pruple;' title='以服务器时间为准，未获得服务器时间之前，此选项不可用。启用智能加速模式时，在非正点附近时（大于0小于59分）按照正常速度刷新；当在正点附近时（大于等于59分时），暂停刷新并等到正点即刻刷新。'><label><input disabled='disabled' type='checkbox' id='chkSmartSpeed' />整点准时刷新</label></span>" +
		"</td></tr>" +
		"<tr class='append_row'><td id='filterFunctionRow' colspan='9'>" +
		"<span style='font-weight:bold;color:red;'><label title='隐藏完全无票的车次'><input type='checkbox' id='chkFilterNonBookable' />隐藏完全无票的车次</label></span>" +
		"<span style='font-weight:bold;margin-left:10px;color:red;'><label title='隐藏无所需席别的车次'><input type='checkbox' id='chkFilterNonNeeded' />隐藏无所需席别的车次</label></span>" +
		"<span style='font-weight:bold;margin-left:10px;color:blue;'><label><input type='checkbox' id='chkFilterByTrain' />开启按车次过滤</label></span>" +
		"</td></tr>" +
		"<tr><td colspan='9' id='opFunctionRow'><input type='button' class='fish_button' disabled='disabled' value='停止声音' id='btnStopSound' /><input  type='button' class='fish_button' disabled='disabled'  value='停止刷新' id='btnStopRefresh' /><input  type='button' class='fish_button' type='button' value='设置' id='configLink' /> <input type='button' class='fish_button' id='resetSettings' value='清空助手设置' /> <input type='button' class='fish_button configLink' value='IE登录' /> <span style='margin-left:20px;color:purple;font-weight:bold;' id='serverMsg'></span></td> </tr>"
	);
...
<tr class='fish_sep'><td style='text-align:center;' colspan='4'><a href='http://www.fishlee.net/soft/44/' target='_blank' style='color:purple;'>12306.CN 订票助手</a> | <a href='"+ utility.getUpdateUrl() + "' target='_blank' style='color:purple;'>下载新版</a> | 当前版本 v" + window.helperVersion + "</td></tr>\
		</table></div></div>");
	...
	function countDownTimer() {
		...
		$("#refreshtimer").html("[" + (isSmartOn ? "等待整点," : "") + str + (str.indexOf('.') == -1 ? ".0" : "") + "秒后查询...]");
...
<label><input type='checkbox' id='autoCorrentDate' checked='checked' /> 查询日期早于或等于今天时，自动修改为明天</label>\
	...
	(function () {
		var html = "\
<tr class='fish_sep caption'><td><label><input type='checkbox' id='swWhiteList' checked='checked' /> 车次白名单</label></td><td style='font-weight:normal;' colspan='2'>加入白名单的车次，将不会被隐藏(仅为搭配黑名单)</td><td style='text-align:rigth;'><button class='fish_button' id='btnAddWhite'>添加</button><button class='fish_button' id='btnClearWhite'>清空</button></td></tr>\
<tr class='fish_sep'><td colspan='4' id='whiteListTd'></td></tr>\
<tr class='fish_sep caption'><td><label><input type='checkbox' id='swBlackList' checked='checked' name='swBlackList' />车次黑名单</label></td><td style='font-weight:normal;' colspan='2'>加入黑名单的车次，除非在白名单中，否则会被隐藏</td><td style='text-align:rigth;'><button class='fish_button' id='btnAddBlack'>添加</button><button class='fish_button' id='btnClearBlack'>清空</button></td></tr>\
<tr class='fish_sep'><td colspan='4' id='blackListTd'></td></tr>\
<tr class='caption autoorder_steps fish_sep' id='selectPasRow'><td colspan='3'><span class='hide indicator'>① </span>自动添加乘客 （为加入此列表的乘客自动提交订单，<strong>最多选五位</strong>）</td><td><input type='button' class='fish_button' onclick=\"self.location='/otsweb/passengerAction.do?method=initAddPassenger&';\" value='添加联系人' /> <input type='button' class='fish_button' id='btnRefreshPas' value='刷新列表' /></td></tr>\
...
<tr class='caption autoorder_steps fish_sep highlightrow'><td class='name autoordertd'><label style='display:none;color:red;'><input type='checkbox' id='autoorder'/>自动提交订单</label></td><td class='autoordertd' colspan='3'><p style='display:none;'><img id='randCode' src='/otsweb/passCodeAction.do?rand=randp' /> <input size='4' maxlength='4' type='text' id='randCodeTxt' /> (验证码可在放票前填写，临近放票时建议点击图片刷新并重新填写，以保证成功。)</p></td></tr>\
";
		...
		function appendTrainCodeToList(target) {
			var code = prompt("请输入您要加入列表的车次。车次可以使用正则表达式（不知道的话请直接填写车次编号），比如 【.*】(不包括【】号) 可以代表所有车次，【K.*】可以代表所有K字头的车次，【D.*】可以代表所有D字头车次等等");
			if (!code) return;
	...
	(function () {
		...
		html.push("剩余票数小于所填数字，就不处理</td></tr>");
	...
	(function () {
		...
		$("#filterFunctionRow").append("<label style='font-weight:bold;color:red;margin-left:10px;'><input type='checkbox' id='closeFuseSearch'>隐藏发站不完全匹配的车次</label><label style='font-weight:bold;color:red;margin-left:10px;'><input type='checkbox' id='closeFuseSearch1'>隐藏到站不完全匹配的车次</label>");
		$("#closeFuseSearch, #closeFuseSearch1").parent().attr("title", '默认情况下，查找‘北京’时，会包括‘北京西’这个车站。勾选后，将不显示‘北京西’。');
	...
	(function () {
		$("#helpertooltable tr:last").before("<tr class='fish_sep'><td class='name'>保持在线</td><td colspan='3'>助手每隔十分钟会帮你刷新，防止掉线。最后刷新时间：<strong id='lastonlinetime'>无</strong></td></tr>");
...
");
		...
		$("#profile_delete").click(function () {
			...
			if (!idx || !confirm("确定要删除此出行模式？")) return;
...
function initLogin() {
	...
	$(".enter_right").empty().append("<div class='enter_enw'>" +
		...
		"<li> <a href='javascript:;' class='configLink' tab='tabLogin'>登录设置</a> | <a href='" + utility.getUpdateUrl() + "' target='_blank' style='style='color:purple;''>下载新版</a></li>" +
	...
	html.push("<table><tr><td style='width:33%;font-weight:bold;background-color:#f5f5f5;'><strong>您还可以通过以下网址访问订票网站：</strong></td><td style='font-weight:bold;background-color:#f5f5f5;'>版本信息</td></tr>");
	...
	html.push("</ul></td><td><ul>版本：<strong>" + window.helperVersion + "</strong></ul></td></tr></table>");
	...
	var html = ["<li style='color:blue;'>网络和电话订票提前20天，本日起售【<u><strong>"];
	html.push(utility.formatDate(addDays.call(curDate, 19)));
	html.push("</strong></u>】车票<br />代售点和车站提前18天，本日起售【<u><strong>");
	html.push(utility.formatDate(addDays.call(curDate, 17)));
	html.push("</strong></u>】车票<br />【<a href='javascript:;' id='querySaleDate'>根据乘车日期推算起售日期</a>】【<a href='http://www.12306.cn/mormhweb/zxdt/tlxw_tdbtz53.html' target='_blank'>预售期公告</a>】");
